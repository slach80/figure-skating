...
import { SkaterList, SkaterDetail, PaginatedResponse, CompetitionHistoryItem } from '@/types/skater'
import { SkaterStatsProfile } from '@/types/competition'

const STALE_TIME = 5 * 60 * 1000 // 5 minutes

export function useSkaters(page: number = 1) {
  return useQuery({
    queryKey: ['skaters', page],
    queryFn: async () => {
