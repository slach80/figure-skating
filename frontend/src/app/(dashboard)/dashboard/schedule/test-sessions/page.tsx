  ...
  return (
    <tr className="border-t border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900">
      <td className="px-4 py-2.5">
        <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{reg.skater_name}</p>
        <p className="text-xs text-slate-500">{reg.skater_usfs || '—'}</p>
      </td>
      <td className="px-4 py-2.5">
        <div className="flex flex-wrap gap-1">
          {reg.test_types.map(t => (
            <span key={t} className="text-xs bg-primary/10 dark:bg-primary/20 text-primary px-1.5 py-0.5 rounded">
      ...
      <td className="px-4 py-2.5">
        <select
          ...
          className="text-xs border border-slate-300 dark:border-slate-600 rounded px-2 py-1 text-slate-700 dark:text-slate-300 disabled:opacity-50"
        >
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">{session.name}</h3>
              {session.is_registration_open ? (
                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:text-emerald-400 font-medium">Open</span>
              ) : (
                <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 font-medium">Closed</span>
              )}
            </div>
          ...
          <div className="flex-shrink-0 flex items-center gap-4">
            ...
            <div className="text-right hidden sm:block">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">${session.fee_per_test}</p>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200">New Test Session</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Session Name *</label>
          <input
            type="text"
            value={form.name}
            onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            placeholder="e.g. Spring 2026 Test Session"
            className="w-full border border-slate-300 dark:border-slate-600 rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
            autoFocus
          />
        </div>
        ...
        {/* Judge */}
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Judge Name</label>
          <input
            type="text"
        ...
        {/* Fee per test */}
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Fee Per Test ($)</label>
          <input
            type="text"
        ...
        {/* Max registrations */}
        <div>
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Max Registrations</label>
          <input
            type="number"
        ...
        {/* Test types */}
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Test Types Offered</label>
        ...
        <div className="sm:col-span-2">
          ...
          <textarea
            ...
            className="w-full border border-slate-300 dark:border-slate-600 rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none"
          />
        </div>
        ...
        <div className="sm:col-span-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="rounded border-slate-300 dark:border-slate-600 text-primary focus:ring-primary/30"
            />
            <span className="text-sm text-slate-700 dark:text-slate-300">Open for registration</span>
          </label>
        </div>
      </div>

      {create.isError && (
        <p className="text-sm text-red-600 dark:text-red-400">Failed to create session. Please try again.</p>
      )}

      <div className="flex justify-end gap-2 pt-1">
        <button
          onClick={onDone}
          className="px-4 py-1.5 text-sm text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-900"
        >
          Cancel
        </button>
        <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Test Sessions</h1>
