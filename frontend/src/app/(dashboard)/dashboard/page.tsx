...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1>Dashboard</h1>
        <p className="text-slate-600 mt-1 text-sm">Welcome to Line Creek FSC management platform</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard label="Total Members" value={stats?.total_members} icon={TrendingUp} colorClass="text-slate-700" isLoading={statsLoading} />
      </div>

      {recentMembers && recentMembers.results.length > 0 && (
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 className="text-base font-semibold text-slate-900">Recent Members</h2>
