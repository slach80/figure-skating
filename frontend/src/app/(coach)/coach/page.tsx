...
function BookingCard({ booking }: { booking: BookingList }) {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-xl border-2 ${cfg.color.split(' ').find(c => c.startsWith('border')) ?? 'border-slate-200'} p-5 space-y-3`}>
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-lg font-semibold text-slate-900 dark:text-slate-100">{booking.skater_name}</p>
      ...
      {!cancelling && (booking.status === 'pending' || booking.status === 'confirmed') && (
        <div className="flex gap-2 pt-1">
          ...
          <button
            ...
            className="px-3 py-1.5 text-sm border border-red-300 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/40"
          >
            Cancel
          </button>
        </div>
      ...
      {cancelling && (
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={() => setCancelling(false)} className="px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-400 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-900">
            Keep
          </button>
        </div>
      )}
...
export default function CoachTodayPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Today&apos;s Schedule</h1>
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm rounded-xl px-4 py-3">
      ...
      {!isLoading && bookings.length === 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-12 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400 font-medium">No lessons scheduled today</p>
      ...
      {pending.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-yellow-700 dark:text-yellow-400 uppercase tracking-wide mb-3">Awaiting confirmation ({pending.length})</h2>
      ...
      {confirmed.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wide mb-3">Confirmed ({confirmed.length})</h2>
          <div className="space-y-3">
